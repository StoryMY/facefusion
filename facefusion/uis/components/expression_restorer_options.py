...
EXPRESSION_RESTORER_SOURCE_DROPDOWN : Optional[gradio.Dropdown] = None
...
def render() -> None:
	...
	global EXPRESSION_RESTORER_SOURCE_DROPDOWN
	...
	EXPRESSION_RESTORER_SOURCE_DROPDOWN = gradio.Dropdown(
		label = translator.get('uis.source_dropdown', 'facefusion.processors.modules.expression_restorer'),
		choices = expression_restorer_choices.expression_restorer_sources,
		value = state_manager.get_item('expression_restorer_source'),
		visible = has_expression_restorer
	)
	...
	register_ui_component('expression_restorer_source_dropdown', EXPRESSION_RESTORER_SOURCE_DROPDOWN)
...
def listen() -> None:
	EXPRESSION_RESTORER_SOURCE_DROPDOWN.change(update_expression_restorer_source, inputs = EXPRESSION_RESTORER_SOURCE_DROPDOWN)
	...
	if processors_checkbox_group:
		processors_checkbox_group.change(remote_update, inputs = processors_checkbox_group, outputs = [ EXPRESSION_RESTORER_MODEL_DROPDOWN, EXPRESSION_RESTORER_FACTOR_SLIDER, EXPRESSION_RESTORER_AREAS_CHECKBOX_GROUP, EXPRESSION_RESTORER_SOURCE_DROPDOWN ])


def remote_update(processors : List[str]) -> Tuple[gradio.Dropdown, gradio.Slider, gradio.CheckboxGroup, gradio.Dropdown]:
	has_expression_restorer = 'expression_restorer' in processors
	return gradio.Dropdown(visible = has_expression_restorer), gradio.Slider(visible = has_expression_restorer), gradio.CheckboxGroup(visible = has_expression_restorer), gradio.Dropdown(visible = has_expression_restorer)
...
def update_expression_restorer_source(expression_restorer_source : ExpressionRestorerSource) -> None:
	state_manager.set_item('expression_restorer_source', expression_restorer_source)
